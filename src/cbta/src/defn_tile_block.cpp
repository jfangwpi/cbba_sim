...
void TileBlock::cbta(){
	...
	//	std::cout << "w_sol" << std::endl << w_sol << std::endl;
		...
		void (TileBlock::*fcn_cone)(double w, double d,
									const RowVectorXd& xSmp,
									const Matrix<double,2,N_CBTA_W>& btaSmp,
									const Matrix<double,2,1>& returnMatrix);
		if (tile->traversal_type(H-1,0)==1)
			fcn_cone = &TileBlock::cbta_s1;
		else
			fcn_cone = &TileBlock::cbta_s2;
	...
	//		std::cout << "qq = " << qq << std::endl;
			...
			const RowVectorXd& xSmp = x_smp.row(H-1);
			const Matrix<double,2,N_CBTA_W>& btaSmp = bta_smp.bottomRows(2);
			Matrix<double,2,1> alfa_in;
			alfa_in << -T_INFINITY, T_INFINITY;
			const Matrix<double,2,1>& alfa = alfa_in;
		...
		for (int pp = H-1; pp > 0; pp--){
			...
			for (int qq = 0; qq < N_CBTA_W_SOL; qq++){
					...
					const RowVectorXd& xSmp = x_smp.row(pp-1);
					const Matrix<double,2,N_CBTA_W>& btaSmp = bta_smp.middleRows(2*pp-2,2);
					Matrix<double,2,1> alfa_in;
					alfa_in << -T_INFINITY, T_INFINITY;
					const Matrix<double,2,1>& alfa = alfa_in;
					(this->*fcn_cone)(w,d,xSmp,btaSmp,alfa);
...
void TileBlock::cbta_s1(double w, double d,
		                const RowVectorXd& xSmp,
		                const Matrix<double,2,N_CBTA_W>& btaSmp,
		                const Matrix<double,2,1>& return_alfa_in){

	Matrix<double,2,1> return_alfa = return_alfa_in;
...
void TileBlock::remove_inf_values(const MatrixBase<Derived1>& v,
								  const MatrixBase<Derived2>& notInfIndex_in){
	
	MatrixBase<Derived2> notInfIndex = notInfIndex_in;
...
void TileBlock::find_zeros(const MatrixBase<Derived_a>& fSmp, const MatrixBase<Derived_b>& idx_zeros_in){

	MatrixBase<Derived_b> idx_zeros = idx_zeros_in;
...
std::vector<double> polyfit(const RowVectorXd& x_data, const RowVectorXd& y_data, const int n){
...
void polyval(std::vector<double> coeffs, const RowVectorXd& x_data, const RowVectorXd& y_out_in){

	RowVectorXd y_out = y_out_in;
...
void TileBlock::interp_broken_seg(const RowVectorXd& x_data,
	                   			  const Matrix<double,2,Dynamic>& y_data,
	                   			  const RowVectorXd& x_interp,
	                   			  const Matrix<double,2,Dynamic>& y_interp_in){

	Matrix<double,2,Dynamic> y_interp = y_interp_in;
	...
	if ((int)y_noInf_inds(0) != -1){ // index of y_data with no Infs is not empty
		int idx1 = 0;
		int idx2 = 0;
...
int TileBlock::find_sample(const RowVectorXd& ySmp, double y){
...
void TileBlock::cbra(int idx_r_from,REGION_BD &REGION_BD,double r_min,
		             ...
		             const RowVectorXi& region_target_2,
		             const RowVectorXi& region_neighbors_in){

	RowVectorXi region_neighbors = region_neighbors_in;
